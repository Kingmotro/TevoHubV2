...
public class FunGunGadget extends Gadget implements VanityItem{
	...
	public void handleEvent(Event e) {
		...
		if (e instanceof ProjectileLaunchEvent)
		{
			...
			if (p.getItemInHand().getItemMeta().getDisplayName().equalsIgnoreCase(getItem().getItemMeta().getDisplayName()))
			{
				event.getEntity().setMetadata("Ability", new FixedMetadataValue(TevoHubV2.getInstance(), "Fun Gun"));
			}
		}
	...
	public ItemStack getActiveIcon() {
		return new ItemStackFactory().createItemStackwithGlow(CC.tnDisable + CC.fBold + "Unequip " + CC.tnGUIHead + getItemName(), Items.DIAMONDBARDING);
	}
